...
%% Peclet Number and Flash Temperature for 3003 Al

b = 0.0002;                                                  % half width of contact (m)
Fc = 90;                                                     % Cutting force in Newtons
Vc = 3*1.7;                                                      % Cutting Speed in (m/s)
w = 0.005;                                                    % Width in (m)
cPi = 893;                                                   % Specific Heat (J/(kg*C))
ki = 154;                                                    % Thermal Conductivity (W/(m-k))
rhoi = 2730;                                                 % density (kg/m^3)
...
plot(x_b, Tsub);
title('Thermal field of 3003 Al');
xlabel('x_b')
ylabel('T');